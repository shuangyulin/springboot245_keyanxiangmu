        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '项目管理员身份证号' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.xiangmuguanliyuanIdNumber" placeholder="项目管理员身份证号" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        <el-button
                                v-if="isAuth('xiangmuguanliyuan','新增')"
                                type="success"
                                icon="el-icon-plus"
                                @click="addOrUpdateHandler()"
                        >新增</el-button>
                        <el-button
                                v-if="isAuth('xiangmuguanliyuan','删除')"
                        ...
                        >删除</el-button>
                        <el-button
                                v-if="isAuth('xiangmuguanliyuan','报表')"
                                type="success"
                                icon="el-icon-pie-chart"
                                @click="chartDialog()"
                        >报表</el-button>
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('xiangmuguanliyuan','导出')" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "统计.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="primary"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                          ...
                          v-if="isAuth('xiangmuguanliyuan','查看')"
                          :data="dataList"
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="xiangmuguanliyuanPhoto"
                                     header-align="center"
                                     width="200"
                                     label="头像">
                        <template slot-scope="scope">
                            <div v-if="scope.row.xiangmuguanliyuanPhoto">
                                <img :src="scope.row.xiangmuguanliyuanPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="xiangmuguanliyuanEmail"
                                   header-align="center"
                                   label="电子邮箱">
                        <template slot-scope="scope">
                            {{scope.row.xiangmuguanliyuanEmail}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="xiangmuguanliyuanAddress"
                                   header-align="center"
                                   label="住址">
                        <template slot-scope="scope">
                            {{scope.row.xiangmuguanliyuanAddress}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('xiangmuguanliyuan','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                            <el-button v-if="isAuth('xiangmuguanliyuan','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'修改':'' }}</el-button>
                            <el-button v-if="isAuth('xiangmuguanliyuan','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'删除':'' }}</el-button>

                            <el-button v-if="isAuth('xiangmuguanliyuan','修改')" type="success" icon="el-icon-tickets" size="mini" @click="resetPassword(scope.row.id)">{{ contents.tableBtnFont == 1?'重置密码':'' }}</el-button>
                        </template>
                    </el-table-column>
                </el-table>
                <el-pagination
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                ...
                username : null,
                password : null,
                xiangmuguanliyuanName : null,
                xiangmuguanliyuanPhone : null,
                xiangmuguanliyuanIdNumber : null,
                xiangmuguanliyuanPhoto : null,
                sexTypes : null,
                xiangmuguanliyuanEmail : null,
                xiangmuguanliyuanAddress : null,
                xiangmuguanliyuanDelete : null,
                createTime : null,
            },
            dataList: [],
            pageIndex: 1,
        ...
        methods: {
            ...
            getDataList() {
                ...
                params['xiangmuguanliyuanDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "xiangmuguanliyuan/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            resetPassword(id) {
                //重置密码
                this.$http({
                    url: "xiangmuguanliyuan/resetPassword?id=" + id,
                    method: "get",
                    // id:id
                }).then(({data}) => {
                    if(data && data.code === 0){
